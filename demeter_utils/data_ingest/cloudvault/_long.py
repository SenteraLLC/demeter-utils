...
#         if df_asset_analytics is not None:
#             df_asset_analytics.insert(0, "site_name", asset_name)
#             df_analytic_list = (
#                 pd_concat([df_analytic_list, df_asset_analytics], axis=0, ignore_index=True)
#                 if len(df_asset_analytics.columns) != 0
#                 else df_asset_analytics.copy()
#             )
...
#             filter(
#                 lambda x: (
#                     any([x.startswith(s) for s in col_starts_with_allowable]) and x not in primary_keys + cols_ignore
#                 ),
